...
The product hierarchy and golden fixture journey are defined in `docs/planning/SIMSUITE_APP_DIRECTION_EXECUTION_PLAN_V1.md`. The profile migration and API contract are defined in `docs/planning/GAME_INSTALLATION_PROFILE_V1_DESIGN.md`. The hidden transaction proof still exists only inside Rust tests for one temporary file. Its reconciliation layer classifies known interruption states from existing plan/index/result/restore metadata plus verified backup and live fixture bytes, repairs only already-observed post-move metadata gaps, and can finish deletion of an already-restored exact duplicate after interrupted undo. A separate hidden resume proof consumes only `ResumeFromVerifiedBackupRequired`: it binds to the same verified backup-chain identities, re-reads all evidence, repeats exact source/backup/destination checks immediately before the existing move primitive, and fails closed on injected destination, source, backup, or metadata races. A standalone macOS fixture experiment proves a stronger destination-claim candidate using `std::fs::hard_link`: an existing destination is refused, 16 simultaneous contenders yield exactly one successful claim, the destination is verified before source unlink, and forced verification failure removes only the newly-created link while retaining source. This candidate is still not wired into the transaction. A follow-up persistence/capability audit has now answered the next gate: the existing ApplyPlan run/result/restore foundation is not sufficient to prove before mutation that one exact per-file no-replace claim belongs to this exact run without stretching record semantics. A saved item plus verified backup proves plan/recovery state, but not that this run actually began that item; `pending_log` remains a result-log status and must not become an undocumented ownership journal. Future adoption therefore needs a dedicated per-item attempt identity written before the first user-file mutation, bound to exact validated paths, expected hash/size, verified backup-record identities, the selected claim strategy, and the capability evidence that allowed it. The current filesystem capability model also lacks explicit hard-link support and source/destination same-volume proof. Finally, a hard-link claim does not freeze content because both names share the same underlying file during the two-name interval, so concurrent content writes remain a separate fail-closed safety problem. No schema, sidecar journal, executor, or production behavior was added by this audit. The next Phase D work should design/prove the per-item attempt-journal contract and controlled capability-probe contract separately, obtain equivalent Windows/Linux native evidence rather than infer it, and only then consider transaction integration before moving on to only-affected Library index refresh, multi-file rollback behavior, and the golden `.package`, `.ts4script`, archive, Tray, and curated special-mod fixtures. Real Apply and Restore remain unavailable.
