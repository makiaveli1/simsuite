...
The product hierarchy and golden fixture journey are defined in `docs/planning/SIMSUITE_APP_DIRECTION_EXECUTION_PLAN_V1.md`. The profile migration and API contract are defined in `docs/planning/GAME_INSTALLATION_PROFILE_V1_DESIGN.md`. The hidden transaction proof still exists only inside Rust tests for one temporary file. Its reconciliation layer now classifies the known interruption states from existing plan/index/result/restore metadata plus verified backup and live fixture bytes, without adding schema. It can repair only already-observed post-move metadata gaps and can finish deletion of an already-restored exact duplicate after interrupted undo; both actions are idempotent and require exact scope/path/hash/size agreement. The verified-backup/source-unmoved state remains an explicit resume-required no-op, while tampered, conflicting, or cross-scoped evidence fails closed. The next Phase D work should remain fixture-only and prove a safe resume-from-verified-backup execution protocol plus destination-race hardening, then address only-affected index refresh, multi-file rollback behavior, and the golden `.package`, `.ts4script`, archive, Tray, and curated special-mod fixtures before any real Apply or Restore surface is considered.
