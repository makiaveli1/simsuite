...
The product hierarchy and golden fixture journey are defined in `docs/planning/SIMSUITE_APP_DIRECTION_EXECUTION_PLAN_V1.md`. The profile migration and API contract are defined in `docs/planning/GAME_INSTALLATION_PROFILE_V1_DESIGN.md`. The hidden transaction proof still exists only inside Rust tests for one temporary file. Its reconciliation layer classifies known interruption states from existing plan/index/result/restore metadata plus verified backup and live fixture bytes, repairs only already-observed post-move metadata gaps, and can finish deletion of an already-restored exact duplicate after interrupted undo. A separate hidden resume proof now consumes only `ResumeFromVerifiedBackupRequired`: it requires the same verified backup-chain identities, exact source/backup hash and size, absent destination, exact scope, and no move/undo metadata; then it re-reads all evidence and repeats source/backup/destination checks immediately before the existing move primitive. Injected destination, source, backup, and metadata races fail closed; a successful resume moves once, verifies bytes, records only `pending_log` / `design_only` metadata, is idempotent on repeat, and remains undoable. This narrows the race window but does not make the existing rename/copy primitive atomically no-overwrite. The next Phase D work should remain fixture-only and design/prove an atomic no-replace (or equivalent platform-safe) move boundary, then address only-affected Library index refresh, multi-file rollback behavior, and the golden `.package`, `.ts4script`, archive, Tray, and curated special-mod fixtures before any real Apply or Restore surface is considered.
