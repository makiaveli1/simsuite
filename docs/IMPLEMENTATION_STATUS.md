...
- Game Installation Profile V1 backend foundation: versioned profile/root tables, stable root IDs, idempotent migration of previously saved Mods/Tray/Downloads paths, one explicit active-profile setting, fail-closed enum and JSON parsing, schema self-repair, and a compatibility `LibrarySettings` view for existing callers. Read-only Tauri and TypeScript APIs list complete profiles, return the selected active profile, and produce transient native-root validation reports through shared database loading. Validation checks absolute paths, metadata state, canonical directory identity, root-local case sensitivity, and symlink observations off the Tauri command thread. It skips foreign-native and Wine/Proton/Lutris path interpretation rather than guessing, never persists the report, and keeps overall game readiness in review until a Sims4Adapter proves root coherence. The old settings command can update only the reserved legacy-migration profile and cannot silently rewrite manually created profiles.
...
The profile foundation now preserves existing users' effective Mods, Tray, and Downloads paths through an idempotent compatibility migration. Complete profiles, the selected active profile, and transient read-only native-root validation reports are available through guarded backend and TypeScript APIs. Generic filesystem evidence can be valid, but overall game readiness remains `needs_review` until the Sims4Adapter proves that the roots belong to one coherent Sims 4 setup. Validation does not persist status or capabilities and does not interpret foreign-native, Wine, Proton, or Lutris paths yet.

Next work:

- introduce the Sims4Adapter boundary for Sims 4 user-data coherence, Mods and Tray relationships, supported file types, placement rules, and readiness evidence;
- add a read-only Settings profile summary before introducing edit controls;
