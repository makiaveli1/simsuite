...
- Guarded Settings game-profile selection and evidence: the Game setup section lazy-loads only when opened, lists saved profiles, requires an explicit choice and confirmation click, and shows the active profile, host compatibility, configured and canonical roots, generic filesystem evidence, Sims4Adapter rules and evidence strength, blockers, review notes, and truthful degraded states. Only player-confirmed profiles whose operating environment matches the current host can become active. Foreign-native, Wine/Proton/Lutris, unknown-environment, and unconfirmed profiles remain reviewable but cannot be activated. A successful switch updates the active preference, restarts Downloads monitoring, and refreshes folder-dependent workspaces without running Apply or Restore or moving installed Mods or Tray files. Profile creation, editing, and discovery remain unavailable, and validation results are never persisted.
...
Advance **Game Installation Profile V1** from guarded active-profile selection toward explicit manual profile creation and setup controls, while keeping automatic discovery and executor work behind later safety gates. Do not begin executor work from database models alone or from Mac-only evidence.

The profile foundation preserves existing users' effective Mods, Tray, and Downloads paths through an idempotent compatibility migration. Complete profiles, guarded active-profile selection, transient generic root evidence, and Sims4Adapter readiness reports are available through backend and TypeScript APIs. Settings presents the evidence and permits only confirmed, current-host-compatible profiles to become active; foreign-native and adapter-dependent environments remain review-only. The adapter proves coherent Mods/Tray relationships for matching native profiles and shares its content-extension and placement-depth contract with scanner and validation code.

Next work:
- add manual profile creation and confirmation before automatic discovery;
