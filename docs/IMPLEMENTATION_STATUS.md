...
The product hierarchy and golden fixture journey are defined in `docs/planning/SIMSUITE_APP_DIRECTION_EXECUTION_PLAN_V1.md`. The profile migration and API contract are defined in `docs/planning/GAME_INSTALLATION_PROFILE_V1_DESIGN.md`. The hidden transaction proof still exists only inside Rust tests for one temporary file. Its reconciliation layer classifies known interruption states from existing plan/index/result/restore metadata plus verified backup and live fixture bytes, repairs only already-observed post-move metadata gaps, and can finish deletion of an already-restored exact duplicate after interrupted undo. A separate hidden resume proof consumes only `ResumeFromVerifiedBackupRequired`: it binds to the same verified backup-chain identities, re-reads all evidence, repeats exact source/backup/destination checks immediately before the existing move primitive, and fails closed on injected destination, source, backup, or metadata races. A standalone macOS fixture experiment now proves a stronger destination-claim candidate using `std::fs::hard_link`: an existing destination is refused, 16 simultaneous contenders yield exactly one successful claim, the destination is verified before source unlink, and forced verification failure removes only the newly-created link while retaining source. This candidate is not wired into the transaction. Interruption after hard-link claim leaves two names for the same physical file; on the current macOS run the fixture classifier recognizes that exact device/inode pair as `ExactHardLinkPairNeedsReview`, while an ordinary equal-content copy or symlink alias remains `Ambiguous`, and reconciliation deliberately performs no cleanup because current records do not prove SimSuite created the pair. The next Phase D work should remain fixture-only and decide the durable transaction-intent/capability contract needed before adopting any no-replace primitive, obtain equivalent Windows/Linux evidence rather than inferring native behavior, and address concurrent content-write handling before only-affected Library index refresh, multi-file rollback behavior, and the golden `.package`, `.ts4script`, archive, Tray, and curated special-mod fixtures. Real Apply and Restore remain unavailable.
