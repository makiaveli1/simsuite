...
**Insights:** selected Library detail persistence only adds deferred thumbnail payloads. Thumbnails are not indexed, so that write should trigger **no search refresh**. In contrast, the special-mod/profile engine can lazily change searchable embedded names, family hints, resource summaries, and script namespaces. That production path now uses a two-phase batch. SimSuite first inspects every relevant file and prepares merged insight changes without holding a SQLite write transaction. For each indexed row it also captures the exact stored `files.insights` JSON it prepared from. Only after all inspection finishes does it open one short transaction, update every indexed row with a compare-and-set condition requiring that exact old JSON, and commit the whole batch. Any missing/stale row or later database error aborts the transaction, so an earlier update in the same batch is rolled back. Caller-owned in-memory insight objects are changed only after the complete database batch commits; disk-only `file_id=None` files remain memory-only and likewise wait until any indexed siblings commit successfully. Tests prove two changed indexed rows commit together, a forced second-row failure rolls back the first, stale prepared evidence cannot overwrite newer database data, unchanged repeats perform no extra write, and a failing indexed sibling prevents disk-only memory from advancing. This now provides the production source-insight transaction boundary that a future search refresh could eventually join, but smart search is still not wired into it.
...
5. keep the new two-phase special-mod insight batch as the production source-write boundary; if smart search is implemented later, add its exact scoped refresh and readiness advancement inside the same short database transaction rather than creating a second transaction;
