...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`. The module is compiled only under Rust tests. Its sorting bridge starts from the real backend sorting generator and real saved ApplyPlan snapshot, while every sorting item remains `suggest_move`; recommendation never becomes execution authority by rewriting the plan. The original one-file macOS proof still requires test-only authorization bound to the exact plan/item, physical source hash/size, source/destination paths, and verified backup chain, and still proves conservative one-level folder ownership using intent-before-create, one non-recursive `create_dir`, exact device/inode identity, and keep-on-uncertainty cleanup rules. The newer three-file batch proof adds whole-batch read-only preflight before any backup/folder/move, three exact verified backups, a second preflight, then one immutable batch receipt bound to the original stored provenance hash plus ordered item and backup evidence. Files sharing `Mods/Gameplay` share one batch/run folder-ownership record while each physical move still has its own durable attempt record. A deliberately stopped one-file prefix commits truthful Library membership for that file; retry skips it, reuses the same backup IDs, and permits only untouched pending items whose sealed receipt, original authorization, backup chain, current source membership, source bytes, and empty destination still agree. Because the normal live ApplyPlan hash intentionally includes current Library paths, it remains a pre-execution gate; after legitimate partial membership movement, retry/Undo verify the original persisted provenance hash and sealed batch receipt instead of treating SimSuite's own completed move as plan corruption. Undo preflights all started batch items before the first restore, so one tampered destination blocks unrelated restores; successful Undo restores exact files and Library membership before shared-folder cleanup. Player-added content keeps the folder, and stale sources or destination collisions block before backup/folder/move side effects. Current scope remains test-only and macOS-only: three same-row installed Mods files, one shared immediate child destination folder, and same-process controlled partial-stop/retry. Several destination folders in one batch, nested folder creation, real process-crash/reopen batch recovery, automatic coordinator recovery from arbitrary intermediate attempts, representative large-batch execution stress, race-hardened cross-platform production directory removal, native Windows/Linux fixture execution, real confirmation, and user-file Apply/Restore remain disabled.
