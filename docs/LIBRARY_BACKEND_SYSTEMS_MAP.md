...
- Fixture-only ApplyPlan backup prototype logic lives in `src-tauri/src/core/apply_plan_backup_prototype.rs`; it is private backend test/prototype code and is not registered as a command.
...
| Fixture-only backup prototype | backend test/prototype only | `apply_plan_backup_prototype` copies and verifies temporary test files inside a supplied fixture root and records safe result/restore metadata. It is not a Tauri command, not UI/API-exposed, and not a user-file backup system. |
