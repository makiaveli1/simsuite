...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`; the entire module remains compiled only under Rust tests. V3-V7 continue to provide the sealed sorting receipt, three exact verified backups, per-file durable attempt rows, truthful Library handoffs, same-physical-file interrupted-move recovery, fresh-process reconstruction, and external-`SIGKILL` recovery at the three modeled durable boundaries. V8 generalizes the batch folder layer from one shared organization directory to the exact set of destination parents carried by the sealed ApplyPlan items. Whole-batch preflight and restart reconstruction now require every unique destination directory to be an immediate child of the configured Mods root, without requiring all files to share one folder. Before backups or any folder creation, preflight also inspects every existing category entry and requires it to be a real non-symlink directory; an unsafe later folder therefore blocks the entire batch with no earlier folder creation, backup, move attempt, or Library change. The existing fixture directory schema already keys ownership by `apply_plan_run_id + directory_path`, and V8 now uses that shape directly: directory load/state/create/cleanup helpers operate on one exact path, Apply prepares or validates every authorized directory before the first file move, and Undo evaluates every destination directory independently only after all file and Library restoration checks pass. A new real-backend `SelectedFiles` proof uses three `suggest_move` items whose generated destinations are Gameplay -> CAS -> Gameplay. `Mods/Gameplay` starts missing and is non-recursively created with one exact device/inode ownership row shared by both Gameplay files; `Mods/CAS` starts as a pre-existing real directory, receives no ownership row, and is never considered removable by SimSuite. Clean Apply moves all three files and updates Library membership; Undo restores every source and Library path, removes only the exact empty owned Gameplay folder, returns `NotOwned` for CAS, and confirms CAS is still the same physical directory. A partial retry stops after the first Gameplay item and before CAS, then resumes through CAS and the second Gameplay file while reusing the original three backup result IDs, retaining one Gameplay ownership row, and reusing the original completed attempt. A separate player-content case places an unrelated file inside the owned Gameplay directory after Apply; Undo restores the SimSuite-managed files but retains Gameplay as non-empty with that player file untouched, while CAS remains unchanged. Older single-folder, SQLite-reopen, separate-process, and external-kill tests remain active alongside V8 rather than being replaced. All prior receipt/authorization/backup/capability binding, stale/collision preflight, path-alias handling, tampered-Undo veto, and keep-on-uncertainty cleanup remain active. Scope is still test-only and macOS-only: three same-row installed files, two immediate child category folders, one created/owned and one pre-existing/unowned. V8 does not yet prove many-category/large-batch scaling, nested folder creation, a combined multi-folder external-kill scenario, sudden power-loss/fsync durability, race-hardened cross-platform production directory removal, native Windows/Linux execution, real confirmation, or user-file Apply/Restore; all remain disabled.
