...
| ApplyPlan validation/conflict preview | implemented, read-only response v1 | `preview_apply_plan_validation` loads saved draft ApplyPlan records, item snapshots, blockers/signals, current Library file rows, and configured roots to report stale sources, missing sources, missing roots, unsafe destinations, destination conflicts, review-only/duplicate-review blockers, and backup requirements. It does not persist validation state, create folders/backups, call file-changing commands, or mark plans ready for confirmation. |
...
| Validation preview response model | Implemented response-only v1. | Read-only `preview_apply_plan_validation`. | Future Organize saved-plan validation preview UI. | Must not write files, create folders, persist Apply readiness, or mark drafts ready for confirmation until the full Apply Safety Contract exists. |
