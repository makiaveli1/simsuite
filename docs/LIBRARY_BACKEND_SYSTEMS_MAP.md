...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`; the entire module remains compiled only under Rust tests. The real backend sorting generator still supplies the saved three-file `suggest_move` ApplyPlan, while V3-V6 continue to supply whole-batch preflight, three exact verified backups, one immutable batch receipt, per-file durable attempt rows, truthful Library handoffs, one shared batch/run folder-ownership record, and fail-closed reconstruction/recovery across fresh processes. V7 adds a stronger external termination boundary without adding any product executable. The parent launches the current `libtest` executable in a test-only `hold` mode; that worker creates the throwaway file-backed batch, reaches one requested durable state, writes a bounded real-file ready signal containing run ID/attempt ID/state/PID, calls `sync_all` on that signal, and then parks without advancing later items. The parent does not trust that signal as recovery authority: while the worker remains alive it independently opens the saved SQLite database, reconstructs the batch, verifies the exact recorded attempt state and backup chain, and confirms later items remain unstarted. Only then does the parent invoke `Child::kill()`, wait for process termination, and on macOS verify the worker died via `SIGKILL` (`signal 9`). A fresh child is launched only after the old worker is reaped. The forced-kill tests cover `destination_claim_observed`, `destination_verified`, and `source_release_completed` before Library membership commit; recovery stabilizes only item one, another fresh recovery is idempotent, later fresh completion reuses the exact original backup result IDs, and a separate fresh Undo restores all files/memberships before conservative folder cleanup. The adversarial forced-kill case replaces the claimed destination after SIGKILL with an independent same-byte file; fresh recovery rejects it because physical identity differs, leaving the interrupted state, source Library membership, and untouched later items unchanged. The earlier V6 deliberate-exit process tests remain alongside V7 rather than being replaced. The outer parent TempDir remains the cleanup boundary for intentionally persisted child fixture state. All prior exact receipt/authorization/backup/capability binding, same-physical-file proof before source unlink, stale/collision preflight, player-content retention, tampered-Undo veto, path-alias handling, and keep-on-uncertainty folder cleanup remain active. Scope is still test-only and macOS-only: three same-row installed files, one shared immediate child destination folder, and external SIGKILL only after one of the modeled durable boundaries has been independently confirmed. This proves recovery after an actual outside process kill at those boundaries, but not power-loss/fsync durability, kill-at-any-arbitrary-instruction safety, several destination folders in one batch, nested folder creation, representative large-batch execution stress, race-hardened production directory removal, native Windows/Linux execution, real confirmation, or user-file Apply/Restore; all remain disabled.
