...
| Backup/restore/result-log ApplyPlan contract | design-only | `docs/planning/BACKUP_RESTORE_RESULT_LOG_DESIGN_V1.md` recommends future copy-backup-first recovery, explicit restore maps, per-file result logs, and future `apply_plan_runs` / `apply_plan_results` / `apply_plan_restore_entries` tables. No migration, command, backup execution, restore execution, real Apply, or visible file-changing control exists yet. |
...
| `apply_plan_runs` / `apply_plan_results` / `apply_plan_restore_entries` | Future design only. | Not implemented. | Future backup/restore/result-log foundation. | Must stay unimplemented until a DB-only schema foundation sprint; real file-changing Apply remains blocked. |
