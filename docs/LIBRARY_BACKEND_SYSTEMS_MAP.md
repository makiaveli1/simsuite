...
| Dry-run Apply design | design-only | `docs/planning/DRY_RUN_APPLY_DESIGN_V1.md` defines a future read-only `preview_apply_plan_dry_run` response that would classify saved draft items after validation as blocked, skipped, requiring review, requiring backup, requiring confirmation, requiring destination review, or candidate after future safety gates. No command, API, UI, confirmation workflow, Apply, Restore, backup execution, restore execution, or file-changing behavior exists yet. |
