...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`; the entire module remains compiled only under Rust tests. V3-V8 continue to provide immutable sorting receipts, exact verified backup binding, per-file durable attempt rows, truthful Library handoffs, safe single/multi-folder ownership, same-physical-file interrupted-move recovery, SQLite reopen reconstruction, fresh-process recovery, and externally forced `SIGKILL` recovery at the modeled durable boundaries. V9 adds a representative repeated-batch proof without changing those authority rules. A macOS-only fixture creates `36` tiny installed package rows and asks the real backend `SelectedFiles` planner to sort them; deterministic filenames interleave `Gameplay`, `CAS`, and `BuildBuy`, giving `12` generated `suggest_move` destinations in each immediate child category. `CAS` is a pre-existing real folder, while `Gameplay` and `BuildBuy` are missing. Existing whole-batch preflight still checks all item freshness/collisions and every category entry before backups or directory creation. The full representative execution produces one sealed `36`-item receipt and exactly `36` backup operation results, moves all `36` files through their individual journals, and advances every Library row to its generated destination. Folder ownership remains independent of file count: twelve Gameplay files share one run+path ownership row, twelve BuildBuy files share one, and the pre-existing CAS folder has no ownership record. Undo restores all `36` original sources and Library memberships, removes only the exact empty Gameplay and BuildBuy folders, and preserves CAS with the same physical identity. The representative retry proof stops after `13` interleaved items and resumes later; it reuses the exact original `36` backup result IDs, keeps exactly the same two ownership rows, and reuses all already-committed attempt IDs instead of replaying completed moves. A separate late-staleness proof modifies only the final source after the plan exists; whole-batch Apply fails before any backup, created category folder, move-attempt row, or Library change, leaving all earlier sources untouched and CAS preserved. The full proof also records preflight/Apply/Undo elapsed durations as informational test output without turning hardware-dependent timing into a pass/fail SLA. This validates repeated correctness on a modest realistic fixture, not maximum-size performance: the planner permits up to `250` selected items, but V9 does not claim a speed or memory target for 250 large real mods. Still unproven are many-category/max-size performance characteristics, nested folder creation, representative-batch process-kill recovery as a combined case, sudden power-loss/fsync durability, race-hardened production directory removal, native Windows/Linux file execution, production confirmation persistence/UI integration, and user-file Apply/Restore; all remain disabled.
- Fixture-only sorting confirmation authority proof logic now lives separately in `src-tauri/src/core/apply_plan_confirmation_prototype.rs`, and `src-tauri/src/core/mod.rs` exposes it only under `#[cfg(test)]`. This prototype is intentionally database-only and does not call the filesystem, move engine, backup prototype, Tauri commands, or any production executor. It leaves every backend-generated plan item as `suggest_move`; confirmation is a distinct approval ledger rather than a mutation of the recommendation. A temporary test schema stores one confirmation per exact draft run with `issued`, `consumed`, `revoked`, or `expired` state, the immutable ApplyPlan hash, a SHA-256 digest of the approval nonce rather than plaintext, a second SHA-256 binding hash, and canonicalized context JSON. The binding covers the exact plan/run IDs, hash metadata, preview snapshot hash, source-plan kind, exact item/file IDs, current and destination paths/roots/action kinds, and an active Game Installation Profile that must already be valid+confirmed with valid evidence for every root used by the plan. Issuance and first consumption require the current saved plan hash to remain valid; first consumption atomically claims `issued -> consumed` once. Wrong token, another run, a run rebound to another saved plan, changed active profile/root state, stale Library evidence, edited destination, expiry, revocation, or confirmation-row binding tamper fails closed. Recovery is intentionally different from first use: it may verify a previously consumed historical binding against the persisted immutable plan and stored context after truthful execution has made the old live preview stale, but it cannot create new authority. The existing production `apply_plan_runs.confirmation_token` field remains passive/untrusted and unchanged. Nine focused tests plus a source-level no-filesystem/no-Tauri-command guard prove the contract. This confirmation layer is not execution readiness and does not replace disk preflight, verified backup, durable transaction journaling, recovery, or Undo; no production migration/API/UI or real Apply/Restore capability has been added.
