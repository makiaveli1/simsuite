...
- A measurement-only general-listing breakdown on the same 10,000-row fixture isolates the next dominant paged-list cost without changing production SQL. One run measured a 50-row Name page at 62 ms total: the exact count query took about 3 ms, the row-selection/order probe about 4 ms, and the existing relationship-peer-count path about 54 ms while materializing all 10,000 filtered rows. A 100-row Recently Modified page showed the same shape at 63 ms total, about 3 ms count, 5 ms row probe, and 52 ms relationship work over all 10,000 rows. `EXPLAIN QUERY PLAN` reports `USE TEMP B-TREE FOR ORDER BY` for both probes, but on this fixture the full filtered relationship materialization is substantially larger than the measured row-selection/sort cost. Timings are informational and no performance threshold is enforced.
...
- Relationship peer counts still aggregate by materializing the full filtered list set before paged rows are returned. The 10,000-row breakdown now shows this is the dominant measured general-list cost even for 50- and 100-row pages, while the row-selection/sort probe is much smaller on the same fixture. The next optimization should therefore target bounded peer-count computation first and preserve the current meaning of same-folder and same-pack counts under active filters; sort-index work should remain separate until remeasured afterward.
