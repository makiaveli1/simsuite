...
- Fixture-only ApplyPlan backup and restore prototype logic lives in `src-tauri/src/core/apply_plan_backup_prototype.rs`; it is private backend test/prototype code and is not registered as a command.
...
| Backup/restore/result-log ApplyPlan contract | DB-only schema/API foundation | `docs/planning/BACKUP_RESTORE_RESULT_LOG_DESIGN_V1.md` recommends future copy-backup-first recovery. `apply_plan_runs`, `apply_plan_results`, and `apply_plan_restore_entries` now exist as metadata tables with DB-only commands/API. No backup execution, restore execution, real Apply, or visible file-changing control exists yet. |
| Fixture-only backup/restore prototype | backend test/prototype only | `apply_plan_backup_prototype` copies and verifies temporary test backup files, then restores from recorded backup references to temporary fixture targets and records safe result/restore metadata. It is not a Tauri command, not UI/API-exposed, and not a user-file backup or restore system. |
