...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`; the entire module remains compiled only under Rust tests. The real backend sorting generator still supplies the saved `suggest_move` ApplyPlan, while the existing three-file batch layer supplies whole-batch preflight, three exact verified backups, one immutable batch receipt, per-file attempt rows, truthful Library handoffs, and one shared batch/run folder-ownership record. The V5 crash layer adds a separate recovery gate for already-started attempts after file-backed SQLite teardown/reopen. It does not behave like a second Apply path and cannot create attempts for untouched later batch items. Recovery binds the durable attempt back to the exact run/plan/item, sealed receipt, original test-only authorization, expected source/destination/hash/size, verified backup result/restore IDs, capability evidence, and current Library ownership before any repair. A prepared attempt may proceed only from exact source + missing destination. A `destination_claim_observed` or `destination_verified` attempt requires exact bytes at both names and proof that those two names still identify the same physical file; a same-byte independent replacement destination is therefore rejected. Physical identity is checked again immediately before unlinking the source name. A `source_release_completed` attempt requires missing source + exact destination + Library membership still at the authorized source, then recovery commits only the missing membership handoff. `committed` is verification-only, while `recovery_required`, terminal states, mismatched backup/attempt metadata, unexpected source recreation, rogue Library membership, ambiguous bytes, or changed physical identity fail closed. File-backed tests inject all three dangerous mid-move states, drop the original SQLite connection, reconstruct from persisted evidence after reopen, stabilize only item one, prove a second reopen/recovery is idempotent, then allow normal batch execution to start items two and three with the exact original backup IDs; Undo remains exact afterward. Additional offline tamper tests cover source recreation after release, same-byte/different-file destination replacement, backup-byte changes, durable-attempt metadata changes, and Library path changes, and none advances unrelated items. The earlier stale-source/destination-collision preflight, player-content retention, tampered-Undo veto, path-alias handling, provenance/receipt validation, and conservative folder cleanup remain active. Scope is still test-only and macOS-only: three same-row installed files, one shared immediate child destination folder, controlled intermediate attempt states, and SQLite connection close/reopen within one Rust test process. A true separate OS-process kill/relaunch, sudden power-loss/fsync guarantees, several destination folders in one batch, nested folder creation, representative large-batch execution stress, race-hardened production directory removal, native Windows/Linux execution, real confirmation, and user-file Apply/Restore remain disabled.
