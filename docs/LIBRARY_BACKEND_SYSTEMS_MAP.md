...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`; the entire module remains compiled only under Rust tests. The real backend sorting generator still supplies the saved three-file `suggest_move` ApplyPlan, while V3-V5 continue to supply whole-batch preflight, three exact verified backups, one immutable batch receipt, per-file durable attempt rows, truthful Library handoffs, one shared batch/run folder-ownership record, and fail-closed recovery for intermediate attempt states. V6 adds an actual process boundary without adding a product executable: the parent Rust test launches the current `libtest` executable as a child, the crash child creates its file-backed fixture beneath a parent-owned temporary root, deliberately persists only that throwaway fixture, writes a bounded JSON restart manifest containing database path/fixture root/run ID, reaches the requested durable attempt state, and exits with code `73` through `process::exit`, so Rust destructors and in-memory batch state do not survive. Only after that child has terminated does the parent launch a fresh recovery copy of the test executable. That new process reads the manifest, reopens SQLite, reconstructs the batch from persisted run/receipt/ApplyPlan/Library/backup/filesystem evidence, and invokes the V5 recovery gate; no serialized batch object or prior process memory is trusted. Separate-process tests cover `destination_claim_observed`, `destination_verified`, and `source_release_completed` before Library membership commit. Recovery stabilizes only item one; items two and three retain no attempt and exact source/missing-destination state. Another fresh recovery process proves idempotence, a later fresh completion process starts the untouched items and reuses the exact original backup IDs, and a separate fresh Undo process restores all files and Library memberships before conservative shared-folder cleanup. The process-level adversarial case replaces the claimed destination between child exit and recovery with an independent file containing the same bytes; fresh recovery rejects it because physical file identity no longer matches, leaving the interrupted attempt and source Library membership unchanged and later items unstarted. The outer parent TempDir is the cleanup boundary for child-persisted throwaway state. All prior exact receipt/authorization/backup/capability binding, same-physical-file proof before source unlink, stale/collision preflight, player-content retention, tampered-Undo veto, path-alias handling, and keep-on-uncertainty folder cleanup remain active. Scope is still test-only and macOS-only: three same-row installed files, one shared immediate child destination folder, and controlled separate-process exit/relaunch through the Rust test executable. This proves that recovery can reconstruct authority after the old process is actually gone, but not sudden power-loss/fsync guarantees, arbitrary externally forced `SIGKILL` timing, several destination folders in one batch, nested folder creation, representative large-batch execution stress, race-hardened production directory removal, native Windows/Linux execution, real confirmation, or user-file Apply/Restore; all remain disabled.
