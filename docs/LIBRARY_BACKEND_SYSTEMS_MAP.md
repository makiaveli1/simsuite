...
| ApplyPlan validation/conflict preview | designed, not implemented | `docs/planning/VALIDATION_CONFLICT_PREVIEW_V1.md` defines future validation and conflict statuses, read-only command shape, and required checks for stale sources, missing roots, unsafe destinations, destination conflicts, review-only blockers, and backup/restore requirements. No runtime command exists yet. |
...
| Future validation preview model | Not implemented. | Future read-only validation command. | Organize saved-plan validation preview. | Must not write files, create folders, or mark drafts ready for confirmation until the full Apply Safety Contract exists. |
