...
- Fixture-only transaction/recovery, Library-membership handoff, replacement/new-ID restore, and sorting-to-Apply/Undo bridge proof logic lives in `src-tauri/src/core/apply_plan_fixture_transaction_prototype.rs`. The module is compiled only under Rust tests. Its sorting bridge starts from the real backend sorting generator and real saved ApplyPlan snapshot, while every sorting item remains `suggest_move`; recommendation never becomes execution authority by rewriting the plan. The original one-file macOS proof still requires test-only authorization bound to the exact plan/item, physical source hash/size, source/destination paths, and verified backup chain, and still proves conservative one-level folder ownership using intent-before-create, one non-recursive `create_dir`, exact device/inode identity, and keep-on-uncertainty cleanup rules. The three-file batch proof adds whole-batch read-only preflight before any backup/folder/move, three exact verified backups, a second preflight, then one immutable batch receipt bound to the original stored provenance hash plus ordered item and backup evidence. Files sharing `Mods/Gameplay` share one batch/run folder-ownership record while each physical move still has its own durable attempt record. A deliberately stopped one-file prefix commits truthful Library membership for that file; retry skips it, reuses the same backup IDs, and permits only untouched pending items whose sealed receipt, original authorization, backup chain, current source membership, source bytes, and empty destination still agree. The newer restart layer proves that this state survives a real SQLite connection teardown/reopen using a file-backed temporary database: after item one is committed, the old batch object is consumed and the original `Connection` is dropped; a fresh connection reopens the database and a loader given only the fixture lifetime handle plus run ID reconstructs the batch from the persisted run, receipt, ApplyPlan items, Library rows, backups, and live filesystem boundaries. It verifies run/plan identity, receipt item order/count, source/destination/hash/size evidence, one shared destination folder under Mods, backup-root confinement, and each Library row remaining within its authorized source/destination pair before reusing the existing receipt validator. Resume then skips the committed item, preserves its original attempt, reuses the exact three backup IDs, and revalidates pending source bytes/membership and empty destinations before completing them. Pending source changes or a player file appearing at a pending destination while SQLite is closed both block resumed execution without overwriting the changed/player file. A fully completed batch can likewise close/reopen SQLite and start Undo from the reconstructed state, restoring all exact files and Library membership before the same conservative shared-folder cleanup. Because the normal live ApplyPlan hash intentionally includes current Library paths, it remains a pre-execution gate; after legitimate partial membership movement, reopen/retry/Undo verify the original persisted provenance hash and sealed batch receipt instead of treating SimSuite's own completed move as plan corruption. Current scope remains test-only and macOS-only: three same-row installed Mods files, one shared immediate child destination folder, controlled partial progress, and SQLite connection close/reopen within one test process. This does not yet prove an operating-system process kill/relaunch with independently reconstructed application state, arbitrary mid-attempt crash repair, several destination folders in one batch, nested folder creation, representative large-batch execution stress, race-hardened cross-platform production directory removal, native Windows/Linux fixture execution, real confirmation, or user-file Apply/Restore; all remain disabled.
