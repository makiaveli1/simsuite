...
- The same fixture now measures selected-folder content before and after the identity upgrade and records `EXPLAIN QUERY PLAN` evidence. Legacy and current responses remain exactly equal, but SQLite still chooses `idx_files_source_location_depth` for both direct and recursive folder queries rather than `idx_files_installation_parent_identity`. In the fixture, the direct 5,000-file target sits inside a 6,500-row source/depth candidate lane, while the recursive 2,000-file target sits inside a 9,000-row lane. Informational samples were about 239-255 ms legacy versus 210-243 ms current for direct paging, and 186-208 ms legacy versus 147-158 ms current for recursive paging; no timing threshold is enforced.

Risks:

- Selected-folder content identity is correct, but the current prefix expression prevents direct use of the parent-folder identity index. The 10,000-row proof justifies a separate bounded optimization batch: exact parent equality for non-recursive queries and a component-safe indexed strategy for recursive descendants, with legacy fallback preserved.
