...
`docs/planning/BACKUP_RESTORE_RESULT_LOG_DESIGN_V1.md`. It recommends a future
copy-backup-first recovery model with explicit restore maps and per-file result
...
The fixture-only restore prototype now exists as backend test/prototype code.
It restores from recorded backup references to temporary test files only,
records safe result/restore metadata, and remains hidden from UI and Tauri
